...
public class Ajustes extends ActionBarActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvTitulo = (TextView) findViewById(R.id.tv_titulo_ajustes);
        etNombre = (EditText) findViewById(R.id.et_nombre_ajuste);
        etApellidos = (EditText) findViewById(R.id.et_apellidos_ajustes);
        etEmail = (EditText) findViewById(R.id.et_email_ajustes);
        btModificar = (Button) findViewById(R.id.bt_modificar_ajustes);
        btCancelar = (Button) findViewById(R.id.bt_cancelar_ajustes);

        // Obtener los datos del usuario para mostrarlos en los EditText
        new ObtenerDatosWS().execute("obtener_datos_ajustes", usuario);

        // Se implementan los controladores de eventos en los botones
        btModificar.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                new ModificarWS().execute("modificar_ajustes", usuario,
    ...
    private void crearMenuLateral() {
        drawerLayout = (DrawerLayout) findViewById(R.id.contenedor_principal_ajustes);
        listaDrawer = (ListView) findViewById(R.id.menu_drawer_ajustes);
        ...
        drawerToggle = new ActionBarDrawerToggle(
            ...
            public void onDrawerClosed(View drawerView) {
                ...
                ActivityCompat.invalidateOptionsMenu(Ajustes.this);
    ...
    // Clase privada que controla la llamada al servidor para obtener datos personales
    private class ObtenerDatosWS extends AsyncTask<String, Integer, Boolean> {
        ...
        protected Boolean doInBackground(String... params) {
            ...
            Log.d("Obtener datos: ", json.toString()); // Se comprueba el proceso

            try {
                int success = json.getInt(TAG_SUCCESS);

                if (success == 1) {
                    nombreUsuario = json.getString("nombre");
                    apellidosUsuario = json.getString("apellidos");
                    emailUsuario = json.getString("email");
                } else {
                    informacion = getString(R.string.warning_ajustes_nodata);
                }
        ...
        protected void onPostExecute(Boolean resultado) {
            ...
            if (resultado) {
                ...
                etApellidos.setText(apellidosUsuario);
    ...
    private class ModificarWS extends AsyncTask<String, Integer, Boolean> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            pDialog = new ProgressDialog(Ajustes.this);
            pDialog.setMessage(getString(R.string.info_ajustes_datamodify));
            pDialog.setIndeterminate(false);
            pDialog.setCancelable(false);
            pDialog.show();
        }
        ...
        protected Boolean doInBackground(String... params) {
            ...
            parametros.add(new BasicNameValuePair("metodo", params[0]));
            parametros.add(new BasicNameValuePair("id_usuario", params[1]));
            parametros.add(new BasicNameValuePair("nombre", params[2]));
            parametros.add(new BasicNameValuePair("apellidos", params[3]));
            parametros.add(new BasicNameValuePair("email", params[4]));

            JSONObject json = jsonParser.makeHttpRequest(ws_url, "POST", parametros);
            Log.d("Modificar: ", json.toString()); // Se comprueba el proceso

            try {
                int success = json.getInt(TAG_SUCCESS);

                if (success == 1) {
                    informacion = getString(R.string.info_ajustes_modifyok);
                } else {
                    informacion = getString(R.string.warning_ajustes_failmodify);
                }
            } catch (JSONException e) {
                resultado = false;
                informacion = getString(R.string.warning_ajustes_errorconnection);
